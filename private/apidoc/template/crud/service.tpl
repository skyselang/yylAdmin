...
use {$cache.namespace}\{$cache.class_name} as Cache;
...
class {$service.class_name}
{
     ...
     *   @Apidoc\Returned(ref={Model::class}, field="{$custom.field_list}"),
     ...
     * @param int  $id   {$form.controller_title}id、编号
     * @param bool $exce 不存在是否抛出异常
     * @return array|Exception
     * @Apidoc\Query(ref={Model::class}, field="id")
    ...
    public static function info($id, $exce = true)
    {
        ...
        if (empty($info)) {
            ...
            if (empty($info)) {
                if ($exce) {
                    exception(lang('{$form.controller_title}不存在：') . $id);
                }
                return [];
            }
     ...
     * @return array|Exception
     * @Apidoc\Param(ref={Model::class}, withoutField="id,is_disable,is_delete,create_uid,update_uid,delete_uid,create_time,update_time,delete_time")
     */
