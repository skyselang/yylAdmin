...
use app\common\cache\file\GroupCache as Cache;
...
class GroupService
{
     ...
     * 批量修改字段
     */
    public static $updateField = ['remark', 'sort', 'group_unique'];
     ...
     *   @Apidoc\Returned(ref={Model::class}, field="group_id,group_unique,group_name,desc,remark,sort,is_disable,create_time,update_time"),
    ...
    public static function list($where = [], $page = 1, $limit = 10,  $order = [], $field = '', $total = true)
    {
        ...
        if (empty($field)) {
            $field = $pk . ',group_unique,group_name,desc,remark,sort,is_disable,create_time,update_time';
        } else {
            $field = $pk . ',' . $field;
        }
    ...
    public static function export($export_info)
    {
        ...
        $export_info['type']    = FileSettingService::EXPIMP_TYPE_FILE_GROUP;
     ...
     */
    public static function fileLift($group_id, $file_ids = [])
    {
        $model = self::model();
        $pk    = $model->getPk();
        ...
        if (empty($file_ids)) {
            $file_ids = FileModel::where($where)->column($file_pk);
        }
        $where[] = [$file_pk, 'in', $file_ids];

        $res = $file_model->where($where)->update([$pk => 0, 'update_uid' => user_id(), 'update_time' => datetime()]);
