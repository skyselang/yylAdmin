...
use app\common\cache\system\EmailLogCache as Cache;
...
class EmailLogService
{
     ...
     * 批量修改字段
     */
    public static $updateField = ['sender', 'recipient', 'theme', 'content', 'error'];
     ...
     *   @Apidoc\Returned(ref={Model::class}, field="log_id,sender,recipient,theme,error,create_time,update_time"),
     * })
     */
    public static function list($where = [], $page = 1, $limit = 10, $order = [], $field = '', $total = true)
    {
        ...
        if (empty($field)) {
            $field = $pk . ',sender,recipient,theme,error,create_time,update_time';
        } else {
            $field = $pk . ',' . $field;
        }
    ...
    public static function header($exp_imp = 'import')
    {
        ...
        $header = [
            ...
            ['field' => 'sender', 'name' => lang('发件人'), 'width' => 22, 'color' => ''],
            ['field' => 'recipient', 'name' => lang('收件人'), 'width' => 22],
            ['field' => 'theme', 'name' => lang('主题'), 'width' => 18],
    ...
    public static function export($export_info)
    {
        ...
        $export_info['type']    = FileSettingService::EXPIMP_TYPE_SETTING_EMAIL_LOG;

        $field = 'sender,recipient,theme,create_time,update_time,content,error';
        $limit = 10000;
    ...
    public static function clearLog($limit = 10000)
    {
        ...
        $save_day = $setting['email_log_save_time'];
        if ($save_day > 0) {
            $cache = self::cache();
